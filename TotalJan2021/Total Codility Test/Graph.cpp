...
Graph::Graph(const Graph& g)
{
    directed = g.directed;
    vertices = g.vertices;
    adjacencyList = new list<int>[g.vertices+1];
...
void Graph::breadthFirstTraversal(int source)
{
    ...
    for (int i = 1; i <= this->vertices; i++) visited[i] = false;
...
void Graph::depthFirstTraversal(int source, std::vector<bool>& _visited) const
{
  /*
...
bool Graph::isConnected() {
    ...
    findFalse = std::find(std::begin(visited)+1, std::end(visited), false);
